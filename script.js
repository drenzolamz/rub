...
const animateScore = (finalScore) => {
    ...
    const scoreInterval = setInterval(() => {
        if (currentScore >= finalScore) {
            ...
            fin.innerHTML = `🌟 You got <strong>${finalScore}</strong> out of <strong>${quizdata.length}</strong> questions correct! 🎉`;
        } else {
            currentScore += increment;
            fin.innerHTML = `🌟 You got <strong>${currentScore}</strong> / <strong>${quizdata.length}</strong> 🎉`;
        }
